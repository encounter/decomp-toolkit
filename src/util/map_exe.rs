...
use crate::obj::{ObjInfo, ObjReloc, ObjSectionKind, ObjSplit, ObjSymbol, ObjSymbolFlagSet, ObjSymbolFlags, ObjSymbolKind, ObjUnit};
        ...
        for (addr, objs) in sorted_addr_map {
            ...
            if let (Some(obj_name), Some(obj_start)) = (&cur_obj, &cur_start){
                if addr == entries.last().unwrap().addr {
                    ...
                    // some shenanigans happening here with symbols at section ends overlapping with our split ends
                    // target_sec.splits.push(*obj_start, ObjSplit {
                    //     unit: obj_name.clone(),
                    //     end: section_end,
                    //     align: None,
